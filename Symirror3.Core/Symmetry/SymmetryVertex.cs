...
public class SymmetryVertex(SphericalPoint vector, int symmetryElementType, int index) : ISymmetryElement
{
    /// <summary>頂点の位置を取得します。</summary>
    public SphericalPoint Point { get; } = vector;

    /// <summary>同じカテゴリの要素内での、区別のための種別番号を取得します。</summary>
    public int ElementType { get; } = symmetryElementType;
